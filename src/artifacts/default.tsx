...
import { Droplet, Leaf, Sun, Cloud, LayoutDashboard } from 'lucide-react';
...
const Dashboard = () => {
  ...
  const sustainabilityMetrics = useMemo(() => {
    ...
    const waterEfficiency = fields.map(field => {
      ...
      const averageWaterPerAcre = totalWaterUsage / field.size;
    ...
    const organicScore = fields.map(field => {
      ...
      const avgFertilizerPerAcre = fertilizerUse / field.size;
    ...
    const harvestEfficiency = fields.map(field => {
      ...
      const avgHarvestPerAcre = totalHarvest / field.size;
